import React, { Component } from 'react';
import ReactWOW from 'react-wow';
import TokenSystem from './assets/images/token_system.png';
import './assets/styles/Influencers.css';

export class TokeniseCommunities extends Component {
  render() {
    return (
      <div className='Top-Div' style={{ marginTop: 90 }}>
        <ReactWOW animation='slideInLeft'>
          <section className='SlideTwo'>
            <div className='container'>
              <div className='row'>
                <div className='col-md-12'>
                  <div className='Slide-Header'>
                    <h2 className='Slide-Title'>TOKENISE YOUR COMMUNITIES</h2>
                    <p className='Slide-Title-Desc text-center'>
                      3Speak will provide a Decentralised, consensus driven
                      Token creation system for content creators and communities
                      to create their own immutable, un-censorable token
                      economies outside of the system.
                    </p>
                  </div>
                </div>
              </div>
              <div className='row'>
                <ReactWOW animation='flipInX'>
                  <div className='col-md-7'>
                    <img
                      src={TokenSystem}
                      alt='Token System'
                      style={{ width: '100%' }}
                    />
                  </div>
                </ReactWOW>
                <div className='col-md-5'>
                  <div className='Sub-Slide-Header'>
                    <h3 className='Tokenise-Title'>CONSENSUS DRIVEN TOKEN</h3>
                    <div className='row'>
                      <div className='col-12'>
                        <div className='row' style={{ marginBottom: '28px' }}>
                          <div className='col-2'>
                            <img alt='list icon' src='data:image/png;base64,' />
                          </div>
                          <div className='col-10'>
                            Content creator and community economies outside and
                            independent of the system are unstoppable
                          </div>
                        </div>
                        <div className='row'>
                          <div className='col-2'>
                            <img alt='list icon' src='data:image/png;base64,' />
                          </div>
                          <div className='col-10'>
                            These tokens will be ETH wrapped and so will be able
                            to be traded on highly liquid decentralised
                            exchanges on the Ethereum network
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </section>
        </ReactWOW>
      </div>
    );
  }
}

export default TokeniseCommunities;
